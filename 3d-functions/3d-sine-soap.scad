...
function f0(x, y) = sin(sqrt(pow((x-(size/2)), 2) + pow(y, 2)) * 90 / PI) + 2;
function f1(x, y) = sin(sqrt(pow((x+(size/2)), 2) + pow(y, 2)) * 90 / PI) + 1;
//function f1(x, y) = 1;
//function f1(x, y) = sqrt(pow(x, 2) + pow(y, 2) + 3);
//function f1(x, y) = cos(sqrt(pow(x, 2) + pow(y, 2)) * 180 / PI) + 4;
//function f(x, y) = 10 * f0(x, y) / f1(x, y);
function f(x, y) = 2 * (f0(x, y) - f1(x, y));
...
difference() {
  polyhedron(points, faces, convexity = 8);
  translate([0,0,size/2]) cube([size*2, size*2, size/2], center=true);
  translate([0,0,-size/2/2]) cube([size*2, size*2, size/2], center=true);
}
